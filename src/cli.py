...
def grid_search(level, iterations, limit, max_episode_steps, goal_reward, n_eval_episodes,
    ...
    with open(f"{output_folder}/{datetime.now().strftime('%m-%d-%H-%M-%S')}.csv", 'w') as out_file:
        out_file.write(f"seed,{Hyperparameters.csv_header()},{Results.csv_header(iterations)}\n")
        out_file.flush()
        ...
        while limit == 0 or i < limit:
            try:
                ...
                out_file.write(f"{run_seed},{params.csv_params()},{result.csv_result()}\n")
                out_file.flush()

                if result.eval_mean >= goal_reward:
                    model_name = f"{output_folder}/model_gs_{run_seed}_eval{result.eval_mean:.0f}"
                    agent.save(model_name)
                    click.echo(f"  [SAVED] Model saved to {model_name}.zip (eval_mean={result.eval_mean:.1f})")
                del agent
...
@click.option("--seed", default=0, show_default=True, help="Random seed for eval/additional training")
@click.option("--watch/--no-watch", default=True, show_default=True,
              help="Watch rendered episodes after eval")
@click.option("--csv-row", default=None,
...
@click.option("--output-folder", default="output", show_default=True,
              help="Output folder — used to locate consolidated_output.csv header when --csv-row is set")
def run_model(model_path, level, max_episode_steps, goal_reward, n_eval_episodes,
             iterations, seed, watch, csv_row, output_folder):
    ...
    from src.train_model import make_environment, seed_everything, continue_training, eval_and_watch
    ...
    if iterations > 0:
        ...
        result = continue_training(agent, level, iterations, goal_reward=goal_reward,
                                   seed=model_seed, max_episode_steps=max_episode_steps)
    ...
    eval_mean, eval_std, hit_pct = eval_and_watch(
        agent, level, goal_reward=goal_reward, seed=model_seed,
        max_episode_steps=max_episode_steps, n_eval_episodes=n_eval_episodes,
        watch=watch,
    )
    ...
    if iterations > 0:
        ...
        if original_score is None or eval_mean > original_score:
            ...
            click.echo(f"Saved updated model to {updated_name}.zip (improved from {original_score} → {eval_mean:.0f})")
        else:
            click.echo(f"No improvement ({original_score} → {eval_mean:.0f}) — model not saved")
